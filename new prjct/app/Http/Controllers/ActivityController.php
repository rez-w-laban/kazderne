...
class ActivityController extends Controller
{
    ...
    {
        try {
            $activity = Activity::with('user')->with("activityPictures")->findorfail($activity_id);
            return response()->json([
                "activity" => $activity,
            ]);
        } catch (Exception $ex) {
    ...
    public function getCityActivities($city_id)
    {
        try {
            ...
            return response()->json([
                ...
                "status" => "error",
                "message" => "there was an error retrieving activities",
    ...
    public function searchActivities(Request $request)
    {
        try {
            ...
            return response()->json([
                ...
                "status" => "error",
                "message" => "there was an error searching activities",
     ...
     public function nancySearch(Request $request)
    {
        try {
            ...
            if($request->search){
            $search = $request->search;
            $activities = City::where("city_name", "like", "%$search%")
                ->with("Activity")
                ->get();
            }else if($request->activities){
                
                $search = $request->activities;
                $activities = Activity::whereRaw("select * from activities where city_id = ( select city_id from cities where city_name like '%".$search."%' ) ")->get();
